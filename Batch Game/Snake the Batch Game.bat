...
SET /A "map[width]=30", "map[height]=20", "char[0][x]=d[x]=3","char[0][y]=d[y]=3","snake[mass]=1","pel[x]=0","snake[speed]=130","frames=0"

CALL :MAKEMAP %map[width]% %map[height]%

REM --- exit z dodanym rankingiem ---
SET "exit=CALL :SAVE_SCORE& ECHO %ESC%[1;1H%disp[kill]%%ESC%[38;2;255;255;255m%ESC%[%map[height]%;1H%ESC%[3BOuch...Press W to Continue&(COPY NUL "%~dpn0.polsoft")>NUL&EXIT"

DEL "%~dpn0.polsoft" 2>NUL
